...
| `@eslint-react/*` recommended + recommended-type-checked                                                                                             | Native `react/*` + React Compiler category rules (`react/purity`, `react/immutability`, etc.) + `jsx-a11y/*`                          | `approximated` / `lost` for type-checked `@eslint-react` rules | Native ports of `@eslint-react` rules, or JS plugins gain type-aware support       |
